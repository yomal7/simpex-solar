...
document.getElementById('orderSearch').addEventListener('input', function(e) {
    ...
    rows.forEach(row => {
        ...
        const products = row.querySelector('.order-products').textContent.toLowerCase();
        const matches = orderId.includes(searchTerm) || products.includes(searchTerm);
...
function checkEmptyState() {
    ...
    if (visibleRows.length === 0) {
        ...
        const existingEmptyState = tbody.querySelector('.empty-state')?.parentElement;

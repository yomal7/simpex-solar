...
document.addEventListener("DOMContentLoaded", () => {
  ...
  visitForm.addEventListener("submit", (e) => {
    ...
    document.getElementById(
      "scheduledDateTime"
    ).textContent = `Date: ${formatDate(date)} | Time: ${formatTime(time)}`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  completeBtn.addEventListener("click", () => {
    if (
      confirm("Are you sure the installation has been completed successfully?")
    ) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  engineerForm.addEventListener("submit", (e) => {
    ...
    scheduleStatus.className = "visit-status scheduled";
    document.getElementById("statusMessage").textContent =
      "Engineer Inspection Scheduled";
    document.getElementById(
      "scheduledDateTime"
    ).textContent = `Inspection Date: ${formatDate(date)} | Time: ${formatTime(
      time
    )}`;
  ...
  approvalBtn.addEventListener("click", () => {
    if (
      confirm(
        "Confirm that both Engineer Approval and Grid Connection are completed?"
      )
    ) {
      ...
      ).textContent = `Verified on: ${new Date().toLocaleDateString()}`;
    }
  });
...
document.addEventListener("DOMContentLoaded", function () {
  // Get all project phase cards
  const phaseCards = document.querySelectorAll(".phase-card[data-url]");

  // Phase cards click and hover handlers
  phaseCards.forEach((card) => {
    const currentPhase = document.querySelector(".status-tag").classList[1];
    ...
    const cardPhaseIndex = phaseOrder.indexOf(cardPhase);

    // Check if this phase is accessible (current or completed phases)
    const isAccessible = cardPhaseIndex <= currentPhaseIndex;
    ...
    card.addEventListener("click", function (e) {
      ...
      if (this.classList.contains("disabled")) {
        e.preventDefault();
        showFlashMessage("You cannot access this phase yet.", "warning");
        return;
      }
...
// Add CSS styles for flash messages
